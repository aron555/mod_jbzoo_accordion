...
class JFormFieldTemplate extends JFormField {
	...
	public function getInput() {
        ...
        $theme  = JFactory::getApplication();
        $themePath  = JURI::base(true).'/templates/'.$theme->getTemplate().'/';
        $child    = $themePath.'/html/mod_jbzoo_accordion/tmpl';


        if (is_dir($child)) {
            foreach (JFolder::files($child, '^([-_A-Za-z0-9]+)\.php$') as $tmpl) {
